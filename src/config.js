...
(function (global) {
  ...
  const KEYWORDS = [
    // ---------- explicit：招嫖 / 性交易 ----------
    // 广告隐语（正常语境几乎没有，保留 100）：约p/楼凤/莞式/全套/半套/特殊服务
    // 议题词（新闻/社会讨论常见，降 12）：招嫖/嫖娼/卖淫/性交易/色情交易——讨论"卖淫合法化"不杀
    // 双语境词（正常商业/服务用语也常见，降 12-25）：出台/上门服务/明码标价/服务项目/价格表/怎么收费
    // 中频词（广告+讨论都有，降 25）：援交/包养/一夜情/约炮/炮友/外围/伴游
    // 注意：不设「嫖」单字——「白嫖」是 B 站/游戏圈正常用语（免费蹭），单字误杀面太大
    { t: '招嫖', w: 12, c: 'explicit' },
    { t: '嫖娼', w: 12, c: 'explicit' },
    { t: '卖淫', w: 12, c: 'explicit' },
    { t: '賣淫', w: 12, c: 'explicit' },
    { t: '性交易', w: 12, c: 'explicit' },
    { t: '色情交易', w: 12, c: 'explicit' },
    { t: '援交', w: 25, c: 'explicit' },
    { t: '出台', w: 10, c: 'explicit' },
    { t: '包夜', w: 10, c: 'explicit' },
    { t: '包养', w: 6, c: 'explicit' },
    { t: '一夜情', w: 10, c: 'explicit' },
    { t: '約炮', w: 12, c: 'explicit' },
    { t: '约炮', w: 12, c: 'explicit' },
    { t: '约p', w: 12, c: 'explicit' },
    { t: '约P', w: 12, c: 'explicit' },
    { t: '炮友', w: 25, c: 'explicit' },
    { t: '外围', w: 25, c: 'explicit' },
    { t: '外圍', w: 25, c: 'explicit' },
    { t: '外围女', w: 25, c: 'explicit' },
    { t: '伴游', w: 10, c: 'explicit' },
    { t: '伴遊', w: 10, c: 'explicit' },
    { t: '商务伴游', w: 10, c: 'explicit' },
    { t: '楼凤', w: 12, c: 'explicit' },
    { t: '樓鳳', w: 12, c: 'explicit' },
    { t: '莞式服务', w: 12, c: 'explicit' },
    { t: '莞式', w: 12, c: 'explicit' },
    { t: '全套服务', w: 12, c: 'explicit' },
    { t: '全套', w: 12, c: 'explicit' },   // 「全套装备/教材」正常用法，降级靠组合
    { t: '半套', w: 12, c: 'explicit' },
    { t: '特殊服务', w: 12, c: 'explicit' },
    { t: '上门服务', w: 12, c: 'explicit' },
    { t: '性服务', w: 4, c: 'explicit' },
    { t: '坐台', w: 10, c: 'explicit' },
    { t: '卖身', w: 12, c: 'explicit' },
    { t: '明码标价', w: 8, c: 'explicit' },
    { t: '服务项目', w: 8, c: 'explicit' },
    { t: '价格表', w: 8, c: 'explicit' },
    { t: '怎么收费', w: 5, c: 'explicit' },
    { t: '怎么联系', w: 5, c: 'explicit' },
    ...
    { t: '私密照', w: 10, c: 'strong' },
    { t: '私房照', w: 10, c: 'strong' },
    { t: '私密视频', w: 10, c: 'strong' },
    { t: '福利姬', w: 50, c: 'strong' },
    { t: '福利照', w: 50, c: 'strong' },
    { t: '福利图', w: 50, c: 'strong' },
    { t: '福利视频', w: 50, c: 'strong' },
    { t: '深夜福利', w: 12, c: 'strong' },
    { t: '露点', w: 50, c: 'strong' },
    { t: '走光', w: 50, c: 'strong' },
    { t: '无码', w: 50, c: 'strong' },
    { t: '涩图', w: 50, c: 'strong' },
    { t: '色图', w: 50, c: 'strong' },
    { t: '大保健', w: 25, c: 'strong' },   // 调侃语境常见，降级
    { t: '推油', w: 50, c: 'strong' },
    ...
    // ---------- contact：联系方式 / 引流 ----------
    // 用户决策：私我/微信/vx/加微/加V/加v 等正常社交高频词不参与判定（已移除）——
    // 广告识别靠内容词组合（上门/包夜/福利/价格），谐音引流词（微芯/薇信/薇我/微我）保留 25
    { t: '微芯', w: 25, c: 'contact' },
    { t: '薇信', w: 25, c: 'contact' },
    ...
    { t: '联系方式', w: 12, c: 'contact' },   // 中性词「请问联系方式」也常见，降级
    { t: '找我', w: 8, c: 'contact' },
    { t: '找我聊', w: 25, c: 'contact' },
    // 主页类词 25→8：X 上所有博主都会说「来我主页/主页有」是标配话术，不能单杀；靠组合识别
    { t: '主页有', w: 8, c: 'contact' },
    { t: '看我主页', w: 8, c: 'contact' },
    { t: '点我主页', w: 8, c: 'contact' },
    { t: '来我主页', w: 8, c: 'contact' },
    { t: '我的主页', w: 8, c: 'contact' },
    { t: '简介里有', w: 8, c: 'contact' },
    { t: '简介自取', w: 8, c: 'contact' },
    { t: '主页见', w: 8, c: 'contact' },
    ...
    { t: '留邮箱', w: 12, c: 'contact' },   // 「留邮箱发资料」正常服务语境，降级
    { t: '留企鹅', w: 25, c: 'contact' },

    // ---------- baitStrong 14：高置信钓鱼话术（单条即杀）----------
    ...
    // 歧义问句/免费 降 8：想看我的作品、要不要看比赛、视频免费看 等正常语境不杀
    { t: '想看我的', w: 6, c: 'baitWeak' },   // 「想看我的作品」创作者常见，纯辅助信号
    { t: '想不想看', w: 8, c: 'baitWeak' },
    { t: '要不要看', w: 8, c: 'baitWeak' },
    { t: '要看吗', w: 8, c: 'baitWeak' },
    { t: '敢看吗', w: 8, c: 'baitWeak' },
    { t: '你敢看', w: 8, c: 'baitWeak' },
    ...
    { t: '免费看', w: 8, c: 'baitWeak' },   // 「视频免费看」日常语境，降级需组合
    { t: '免费发', w: 8, c: 'baitWeak' },
  ...
  const PATTERNS = [
    ...
    // 微信号：谐音变体（薇信/v信/威信/weixin）+ 账号；vx/wx/微信 前缀不参与（正常社交高频）
    { id: 'wechatId', re: /(?:weixin|v信|薇信|威信)\s*[:：]?\s*[a-zA-Z0-9_-]{4,20}/i, w: 25, label: '微信号' },
    ...
    // 钱包地址已移除：cleanText 去空格后正常英文长句会误匹配 BTC 地址格式，误杀面大
    ...
    // 涩/骚/裸 + 图/照/视频
    // 涩图/裸照类——跨度收紧到 1 字且降 12 不单杀：
    // 「色彩、构图」「配色照片」等审美/设计正常词汇不误杀；涩图/裸照/私密照已有 strong 50 覆盖
    { id: 'lewdPhoto', re: /(涩|色|骚|裸|私密).{0,1}(图|照|照片|视频|片子)/, w: 12, label: '涩图/裸照类' },
    ...
    // 主页引流（加我主页 / 来主页 / 点主页）——博主标配话术，弱信号 5，靠组合
    { id: 'zhuye', re: /(?:加|来|点|进).{0,2}(?:我)?主页/, w: 5, label: '主页引流' },
    // 简介引流（看我简介 / 翻翻我简介）
    { id: 'jianjie', re: /(?:看看|看|翻翻看|翻翻|翻).{0,3}(?:我的|我)?简介/, w: 5, label: '简介引流' },
    // 内容主页负向：出现作品/教程/文章等创作者内容词时，主页类信号抵消（创作者常规话术）
    // 注意不含 资源/合集/分享（广告也常用），且不能太强
    { id: 'contentHome', re: /(作品|教程|文章|干货|内容|更新|新作|图集|设计|代码|博客)/, w: -12, label: '内容主页' },
    // 「加我vx/加我微信/加我qq」口语缩写引流（"加我好友""加我微"不匹配；vx 前缀用户决策移除）
    { id: 'jiawoContact', re: /加我.{0,3}(?:微信|qq|Q|企鹅|薇)/, w: 25, label: '加我联系方式' },
    // 懂的 + 行动词 = 引流
    { id: 'dongdeAct', re: /懂的(?:来|私|加|联系|滴滴)/, w: 25, label: '懂的来引流' },
    // 微信正常语境负向已移除（微信词不参与判定，负向会干扰组合计分）
    // 价格 + 元（「300/2小时」）
    // 按次计价——歧义大（"80美元/次"API定价/"住一晚200块"酒店），降 10 弱信号；
    // 负向后行断言排除 美元/欧元/日元 等外币词
    { id: 'priceUnit', re: /(?:(?<![美欧日港澳新韩])元|块|rmb|RMB)\s*\/?\s*(?:一?小时|次|晚|夜|h|H)|(?:一小时|一次|一晚|一夜|半天)\s*\d{2,4}\s*(?:元|块)/, w: 10, label: '按次计价' },
  ];
  ...
  const NAME_HARD_TERMS = [
    ...
    '伴游', '楼凤', '莞式', '出台', '卖淫', '坐台',
    // 明确引流（vx/wx 单独是正常社交缩写/公司名如 VX数码，不参与）
    '加微信', '薇信', 'telegram', '电报', 'onlyfans',
  ];
